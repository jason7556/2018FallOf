...
void ofApp::setup(){
    ...
    gui.add(xNoise.setup("xNoise",0.001 , 0.001f , 2) );
    gui.add(zNoise.setup("zNoise",0.001 , 0.001f , 2) );
...
void ofApp::draw(){
    ...
    for (int z=  0 ; z< ofGetHeight() ; z+=w){
        for (int x=  0 ; x< ofGetWidth() ; x+=w){
            ...
            float offset = ofMap(d, 0, maxD, -2, 2)*ofNoise(counter, z * zNoise);
            float a = angle + offset;
            float h = floor(ofMap(sin(a), -1, 1, 100, height))*ofNoise(counter,x * xNoise);
